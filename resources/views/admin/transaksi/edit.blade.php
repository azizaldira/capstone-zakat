...
@section('header', 'Edit Transaksi Zakat')
    ...
    <div class="col-lg-8">
        <div class="card shadow mb-4">
            <div class="card-header py-3">
                <h6 class="m-0 font-weight-bold text-primary">Form Edit Transaksi ({{ $transaksi->kode_transaksi }})</h6>
            </div>
            <div class="card-body">
                <form action="{{ route('admin.transaksi.update', $transaksi->id) }}" method="POST">
                    ...
                    <div class="mb-3">
                        ...
                        <select class="form-select @error('muzakki_id') is-invalid @enderror" id="muzakki_id" name="muzakki_id" required>
                            ...
                            @foreach($muzakkis as $muzakki)
                                <option value="{{ $muzakki->id }}" {{ old('muzakki_id', $transaksi->muzakki_id) == $muzakki->id ? 'selected' : '' }}>
                    ...
                    <div class="mb-3">
                        ...
                        <select class="form-select @error('jenis_zakat') is-invalid @enderror" id="jenis_zakat" name="jenis_zakat" required>
                            ...
                            @foreach(['Zakat Fitrah', 'Zakat Mal', 'Infak', 'Sedekah'] as $jenis)
                                <option value="{{ $jenis }}" {{ old('jenis_zakat', $transaksi->jenis_zakat) == $jenis ? 'selected' : '' }}>{{ $jenis }}</option>
                            @endforeach
                        </select>
                        @error('jenis_zakat')
                            <div class="invalid-feedback">{{ $message }}</div>
                    ...
                    <div class="mb-3">
                        ...
                        <input type="number" class="form-control @error('nominal') is-invalid @enderror" id="nominal" name="nominal" value="{{ old('nominal', floor($transaksi->nominal)) }}" placeholder="Contoh: 100000" min="1" required>
                    ...
                    <div class="mb-3">
                        ...
                        <select class="form-select @error('metode_pembayaran') is-invalid @enderror" id="metode_pembayaran" name="metode_pembayaran" required>
                            ...
                            @foreach(['Tunai', 'Transfer Bank', 'E-Wallet'] as $metode)
                                <option value="{{ $metode }}" {{ old('metode_pembayaran', $transaksi->metode_pembayaran) == $metode ? 'selected' : '' }}>{{ $metode }}</option>
                            @endforeach
                        </select>
                    ...
                    <div class="mb-3">
                        ...
                        <input type="date" class="form-control @error('tanggal_bayar') is-invalid @enderror" id="tanggal_bayar" name="tanggal_bayar" value="{{ old('tanggal_bayar', $transaksi->tanggal_bayar->format('Y-m-d')) }}" required>
                        @error('tanggal_bayar')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>

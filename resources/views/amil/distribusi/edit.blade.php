...
@section('header', 'Edit Distribusi Zakat')
    ...
    <div class="col-lg-8">
        <div class="card shadow mb-4">
            <div class="card-header py-3">
                <h6 class="m-0 font-weight-bold text-primary">Form Edit Distribusi ({{ $distribusi->kode_distribusi }})</h6>
            </div>
            <div class="card-body">
                <form action="{{ route('amil.distribusi.update', $distribusi->id) }}" method="POST">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="mb-3">
                        ...
                        <select class="form-select @error('mustahik_id') is-invalid @enderror" id="mustahik_id" name="mustahik_id" required>
                            ...
                            @foreach($mustahiks as $mustahik)
                                <option value="{{ $mustahik->id }}" {{ old('mustahik_id', $distribusi->mustahik_id) == $mustahik->id ? 'selected' : '' }}>
                    ...
                    <div class="mb-3">
                        ...
                        <select class="form-select @error('kategori_bantuan') is-invalid @enderror" id="kategori_bantuan" name="kategori_bantuan" required>
                            ...
                            @foreach(['Zakat Fitrah', 'Zakat Mal', 'Bantuan Pendidikan', 'Bantuan Kesehatan', 'Bantuan Sosial', 'Bantuan Darurat'] as $kategori)
                                <option value="{{ $kategori }}" {{ old('kategori_bantuan', $distribusi->kategori_bantuan) == $kategori ? 'selected' : '' }}>{{ $kategori }}</option>
                            @endforeach
                        </select>
                    ...
                    <div class="mb-3">
                        ...
                        <input type="number" class="form-control @error('nominal_distribusi') is-invalid @enderror" id="nominal_distribusi" name="nominal_distribusi" value="{{ old('nominal_distribusi', floor($distribusi->nominal_distribusi)) }}" placeholder="Contoh: 500000" min="1" required>
                    ...
                    <div class="mb-3">
                        ...
                        <textarea class="form-control @error('keterangan') is-invalid @enderror" id="keterangan" name="keterangan" rows="3">{{ old('keterangan', $distribusi->keterangan) }}</textarea>
                        @error('keterangan')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>

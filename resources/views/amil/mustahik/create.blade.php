...
@section('header', 'Tambah Data Mustahik')
    ...
    <div class="col-lg-8">
        <div class="card shadow mb-4">
            <div class="card-header py-3">
                <h6 class="m-0 font-weight-bold text-primary">Form Tambah Mustahik</h6>
            </div>
            <div class="card-body">
                <form action="{{ route('amil.mustahik.store') }}" method="POST">
                    @csrf
                    ...
                    <div class="mb-3">
                        ...
                        <select class="form-select @error('kategori_asnaf') is-invalid @enderror" id="kategori_asnaf" name="kategori_asnaf" required>
                            ...
                            @foreach(['Fakir', 'Miskin', 'Amil', 'Mualaf', 'Riqab', 'Gharim', 'Fisabilillah', 'Ibnu Sabil'] as $asnaf)
                                <option value="{{ $asnaf }}" {{ old('kategori_asnaf') == $asnaf ? 'selected' : '' }}>{{ $asnaf }}</option>
                            @endforeach
                        </select>
